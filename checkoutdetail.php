		...
		if(isset($_GET['orderid'])&&$_GET['orderid']!='Y')
		{
				...
				?>
					<div class="row">
						<div class="span2">&nbsp;</div>
						<div class="span8">
							<div class="alert alert-danger">Dear Customer, Please go to your dashboard and select setting to add your email, as we need to send you an email to confirm your order and your receipt. You can still make your payment, but we will not be able to send you your receipt.</div>
							...
							<table class="table ordersummary" id="checkoutdetails">
								...
								<tr>
									...
									<td>
										...
										<tr>
											<?php if($row['event_id']=="0"){?>
												<td colspan="2"><a href="update_bar_order.php?id=<?php echo $row['id']?>"><input type="button" class="btn btn-info bg-pink pull-right" value="Update Booking"/></a></td>
												<?php }?>
									
												<?php if($row['bar_id']=="0"){?>
												<td colspan="2"><a href="update_order.php?id=<?php echo $row['id']?>"><input type="button" class="btn btn-info bg-pink pull-right" value="Update Booking"/></a></td>
												<?php }?>
										</tr>
										</table>
									</td>
						...
						<center>
							<table class="table ordersummary" id="bookingordersummary">
								...
								/* 	echo "<pre>";
									...
									<td></td>
									<td>
										<form action="https://mybarnite.com/orders.php" method="post" value="" id="fullpay">
										<?php 
										...
										?>
										<input type="hidden" name="instId"  value="<?php echo $row['id']; ?>">
										<input type="hidden" name="cartId" value="MB<?php echo time(); ?>">
										<input type="hidden" name="currency" value="GBP">
										<input type="hidden" name="MC_totalpayable" value="<?php echo $totalamount * 100;?>">
									...
									<tr>
										<td colspan="2">	
											...
											<p>We are using Stripe technology to collect the details and process your payment on our website.</p>
											
											<p>After confirming your order(s), you will be redirected to Stripe's secure page to complete and submitt your payment for final processing.</p>
											
											<p>Stripe will re-submit your transaction to your bank for authentication, whether or not the transaction is successful or declined, you will received an email confirming the transaction details in the email provided while registering on our website.</p>
										</td>
									</tr>	
									<tr>
									<td colspan="2">
								...
								<tr>
									...
									<td>
										<input type="radio" name="payment_opt" value="1" style="margin:0;" checked> Pay online<br>
								...
								<tr>
									...
									<form action="https://mybarnite.com/orders.php" method="post" value="" id="halfpay">
										<?php 
											$sql2 = "select * from user_register where id=".$_SESSION['id'];
											$exe2 = mysql_query($sql2)or die(mysql_error()); 
											$rowcount2 = mysql_num_rows($exe2);
										...
										<input type="hidden" name="cartId" value="MB<?php echo time(); ?>">
										<input type="hidden" name="currency" value="GBP">
										<input type="hidden" name="MC_totalpayable" value="<?php echo $totalamount * 20;?>">
										<input type="hidden" name="desc" value="">
										<input type="hidden" name="MC_success" value="http://mybarnite.com/">
										<input type="hidden" name="MC_payonline" id="MC_payonline" value="1">
										<input type="hidden" name="MC_pendingamount" id="MC_pendingamount" value="0">
										...
										<script
										    ...
										    data-name="<?php echo $row['name'];?>"
										    data-description=""
										    data-amount="<?php echo $totalamount * 20;?>"
										    data-currency="gbp"
										    data-image = "https://mybarnite.com/images/barlogo.png"
										    data-allow-remember-me = "false">
										</script>      	
									...
									<tr>
										<td colspan="2">	
											...
											<p>After confirming your order(s), you will be redirected to Stripe's secure page to complete and submit your payment for final processing.</p>
...
<script type="text/javascript">
	jQuery(document).ready(function($) {
		$('input[name=payment_opt]').change(function(){
			var value = $( 'input[name=payment_opt]:checked' ).val();
			if(value == 1){
				$('#halfpay').hide();
				$('#fullpay').show();
			}
