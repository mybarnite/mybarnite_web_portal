...
{
	mysql_query("INSERT INTO `blog_category` (`bcat_id`, `blog_category`, `status`) VALUES ('', '".$_REQUEST['cityname']."', 'Y')");
	header("location:blog-category.php?ret=1"); exit;
}

if(isset($_REQUEST['submit']) && $_REQUEST['id'] != '')
{
	mysql_query("UPDATE `blog_category` SET `blog_category` = '".$_REQUEST['cityname']."' WHERE bcat_id = '".$_REQUEST['id']."'");
	header("location:blog-category.php?id=".$_REQUEST['id']."&ret=1"); exit;
}

$getcategory = mysql_fetch_array(mysql_query("SELECT * FROM `blog_category` WHERE bcat_id = '".$_REQUEST['id']."'"));
?>
...
<div class="wrapper">
    ...
    <!-- Content -->
    <div class="content">
    	<div class="title"><h5>Blog Management</h5></div>
        ...
        <form action="" class="mainForm" method="post" name="form1" id="form1">
            ...
            <div style="color:#090; font-weight:bold; padding:5px 0 5px 13px;">Category Added Successfully!</div>
            <?php } ?>
            <div class="rowElem noborder">
            <label>Category</label>
            <div class="formRight"><input name="cityname" type="text" value="<?php echo $getcategory['blog_category']; ?>" />
