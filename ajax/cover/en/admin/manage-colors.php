...

{
	...
	mysql_query("DELETE FROM `shoe_colors` WHERE id = '".$_REQUEST['DelId']."' ");
...
$getQuery = mysql_query("SELECT * FROM `shoe_colors` ORDER BY colors ASC");

if(isset($_REQUEST['colorinput'])){
  
   /* $exist = false;
   
      while($getValue = mysql_fetch_array($getQuery)){
        var_dump($_REQUEST['color_name'] .' -- '. $getValue['colors']);
        //exit;
        if(strtolower(trim($_REQUEST['colors'])) == strtolower(trim($getValue['color_name']))){

            $exist = true;
        }
     }*/
     //if(!$exist){
        $res = mysql_query("INSERT INTO `shoe_colors` (`id`, `uid`, `colors`, `status`) VALUES ('', '0', '".$_REQUEST['color_name']."', 'Y')");
     //}
    ...
    <div class="content">
        ...
        <div class="widget first">
        	...
        	<div class="head">
        	  ...
        	  <h5 class="iFrames">Manage Colors</h5>
            ...
            <div style="padding:5px 10px;">
                <form id="form1" name="form1" method="post" action="">
                  ...
                  <input type="text" name="color_name" id="textfield" style="padding:5px;" placeholder="Enter Color Name" />

                  <input type="submit" name="colorinput" id="button" value="Add New Color" />
                ...
                <thead>
                	...
                	<tr>
                        ...
                        <td width="83%" style="text-align:left; padding-left:8px;">Color Name</td>

                        <td width="17%" align="center" valign="middle">Action</td>
                ...
                <?php //$getQuery = mysql_query("SELECT * FROM `shoe_colors` ORDER BY colors ASC");?>

			     <?php		  while($getValue = mysql_fetch_array($getQuery)) { 
                    if($getValue['colors'] == 'Enter Color Name'){continue;}?> 

                	<tr>

                        <td><?php echo $getValue['colors'];?></td>

                        <td width="17%" align="center" valign="middle">
                        <a href="manage-colors.php?DelId=<?php echo $getValue['id']; ?>" 
