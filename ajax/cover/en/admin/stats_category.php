...
<div class="wrapper">
    ...
    <div class="content">
        ...
        <div class="widget first">
       	  <div class="head"><h5 class="iFrames">Category Sales Report</h5></div>
            <table cellpadding="0" cellspacing="0" width="100%" class="tableStatic">
            	<thead>
                	<tr>
                      <td width="40%" align="left" valign="middle" style="text-align:left; padding-left:8px;">Category</td>
                        <td width="20%">Sales Total</td>
                        <td width="20%">Quantity Total</td>
                        <td width="20%">Total Cost</td>
                    </tr>
                </thead>
                <tbody>
					<?php
                    $getPage = mysql_query("SELECT * FROM category ORDER by id ASC");
						while($getArray = mysql_fetch_array($getPage))
						{
							$getValues = mysql_fetch_array(mysql_query("SELECT * FROM products WHERE category = '".$getArray['id']."'"));
							$showResult = mysql_query("SELECT SUM(amount) FROM transaction WHERE proid = '".$getValues['proid']."'");
							$showQty = mysql_query("SELECT SUM(qty) FROM transaction WHERE proid = '".$getValues['proid']."'");
                    ?>
                    <tr>
                        <td width="40%" align="left" valign="middle"><?php echo $getArray['catname']; ?></td>
                        <td width="20%" align="center"><?php $cartArrayVal = mysql_fetch_array($showResult);  echo 'Rs.'.number_format($cartArrayVal[0]).'.00'; ?></td>
                        <td width="20%" align="center"><?php $showQtyArr = mysql_fetch_array($showQty);  echo number_format($showQtyArr[0]); ?></td>
                      <td width="20%" align="center"><?php echo 'Rs.'.($cartArrayVal[0] * $showQtyArr[0]).'.00' ; ?></td>
                    </tr>
                <?php
                	}
                ?>
