    ...
    <div style="padding:6px 0 0 0;">
      <form id="form1" name="form1" method="post" action="">
        <table width="77%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="12%" align="left" valign="middle">Start Date</td>
            <td width="26%" align="left" valign="middle"><input type="text" name="datepick" id="datepick" <?php if(isset($_REQUEST['datepick']) == '') { ?> value="" placeholder="Click Here to Select Date" <?php } else { ?> value="<?php echo $_REQUEST['datepick']; ?>" <?php } ?> style="padding:6px 6px 7px 6px;" /></td>
            <td width="12%" align="center" valign="middle">End Date</td>
            <td width="28%" align="left" valign="middle"><input type="text" name="datepick2" id="datepick2" value="<?php if(isset($_REQUEST['datepick2']) == '') { echo date("d M, Y"); } else { echo $_REQUEST['datepick2']; } ?>" style="padding:6px 6px 7px 6px;" placeholder="Cliek Here to Select Date" /></td>
        ...
        <div class="widget first">
       	  <div class="head"><h5 class="iFrames">Ordered Products</h5></div>
            <table cellpadding="0" cellspacing="0" width="100%" class="tableStatic">
            	<thead>
                	<tr>
                      <td width="15%" align="left" valign="middle" style="text-align:left; padding-left:8px;">Order ID</td>
                        <td width="43%">Product Name / Payment Mode</td>
                        <td width="15%">Qty</td>
                ...
                <tbody>
					<?php
					if(isset($_REQUEST['datepick']) != '')
					{
                    $getPage = mysql_query("SELECT * FROM transaction WHERE date >= '".$_REQUEST['datepick']."' AND date <= '".$_REQUEST['datepick2']."'");
					}
					else
					{
					$getPage = mysql_query("SELECT * FROM transaction WHERE date >= '".date("d M, Y")."' AND date <= '".date("d M, Y")."'");	
					}
                    ...
                    ?>
                    <tr>
                        <td width="15%" align="left" valign="middle"><?php echo $getArray['transaction_code']; ?></td>
                        <td width="43%" align="center"><?php $productName =  mysql_fetch_array(mysql_query("SELECT * FROM products WHERE proid = '".$getArray['proid']."'")); echo $productName['product_name']; ?></td>
                        <td width="15%" align="center"><?php echo $getArray['qty']; ?></td>
                        <td width="15%" align="center"><?php echo $getArray['amount']; ?></td>
                        <td width="12%" align="center"><?php if($getArray['status'] == 'Y') { echo 'Approved'; } else { echo 'Pending'; } ?></td>
                    </tr>
                <?php
                	}
                ?>
            ...
            <?php if(mysql_num_rows($getPage) < 1) { ?>
          <div style="text-align:center; padding:50px; font-weight:bold;">No Result Found!</div>
			<?php } ?>
        </div>
