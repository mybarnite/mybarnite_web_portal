...
{
	mysql_query("INSERT INTO `addressbook` (`id`, `fullname`, `email`, `phone`, `address`, `datetime`, `status`) VALUES ('', '".$_REQUEST['fullname']."', '".$_REQUEST['email']."', '".$_REQUEST['phone']."', '".$_REQUEST['address']."', now(), 'Y')");
	header("location:add-contact.php?ret=1");
	exit;
}

if(isset($_REQUEST['submit']) && $_REQUEST['eID'] != '')
{
	mysql_query("UPDATE `addressbook` SET `fullname` = '".$_REQUEST['fullname']."', `email` = '".$_REQUEST['email']."', `phone` = '".$_REQUEST['phone']."', `address` = '".$_REQUEST['address']."' WHERE id = '".$_REQUEST['eID']."'");
	header("location:add-contact.php?eID=".$_REQUEST['eID']."&ret=1");
	exit;
}
$getContact = mysql_fetch_array(mysql_query("SELECT * FROM `addressbook` WHERE id = '".$_REQUEST['eID']."'"));
?>
...
<div class="wrapper">
    ...
    <div class="content">
        ...
        <form action="" class="mainForm" method="post" name="form1" id="form1">
            ...
            <div class="head"><h5 class="iList"><?php if($_REQUEST['eID']=='') { ?>Add New<?php } else { ?>Edit<?php } ?> Contacts</h5></div>
			
            <!--<div style="padding:100px; text-align:center;">Under Development</div>-->
            
            <div class="rowElem noborder">
            <label>Full Name:</label>
            	<div class="formRight"><input name="fullname" type="text" id="full_name" value="<?php echo $getContact['fullname']; ?>" /></div></div>
            <div class="rowElem noborder">
            <label>Email:</label>
            	<div class="formRight"><input name="email" type="text" id="email" value="<?php echo $getContact['email']; ?>" /></div></div>
            <div class="rowElem noborder">
            <label>Phone No.:</label>
            	<div class="formRight"><input name="phone" type="text" id="phoneno" value="<?php echo $getContact['phone']; ?>" /></div></div>
			<div class="rowElem noborder">
            <label>Address:</label>
            	<div class="formRight"><input name="address" type="text" id="address" value="<?php echo $getContact['address']; ?>" /></div></div>
            <br />
            <input type="submit" name="submit" value="Save Details" class="greyishBtn submitForm" />
