...
<div class="wrapper">
    ...
    <div class="content">
        ...
        <div class="widget first">
        	<div class="head"><h5 class="iFrames">Placed Orders</h5></div>
          <?php $getRegister = mysql_fetch_array(mysql_query("SELECT * FROM `register` WHERE email = '".$_REQUEST['email']."'"));
	  if($_REQUEST['email'] != '') {
	  	$getTransaction = mysql_query("SELECT DISTINCT orderid FROM orders WHERE uid = '".$getRegister['id']."' AND status = 'Y' ORDER BY id DESC"); 
	  } else {
	  	$getTransaction = mysql_query("SELECT DISTINCT orderid FROM orders WHERE status = 'Y' ORDER BY id DESC"); 
	  }
          ...
          <div class="order_total" style="text-align:right; padding:8px;">O<span>rder</span> T<span>otal</span>: <strong><?php $checkCartVal1 = mysql_query("SELECT SUM(total_price) AS COST FROM `orders` WHERE orderid = '".$orderArray['orderid']."'"); $cartArrayVal1 = mysql_fetch_array($checkCartVal1); ?><strong style="font-size:18px;">Rs. <?php echo number_format($cartArrayVal1[0]); ?></strong></strong></div>
			...
			<?php } } else { ?>
       	  <div class="text_grey" style="text-align:center; padding:50px 0;">No order found with <strong><?php echo $_REQUEST['email']; ?></strong></div>
        <?php } ?>
