...
if($_REQUEST['DelId']!='')
{
	mysql_query("DELETE FROM investment WHERE id = '".$_REQUEST['DelId']."'");
...
<div id="header" class="wrapper">
    <div class="logo"><a href="index.php" title=""><img src="../images/logoB.png" alt="" /></a></div>
    <div class="middleNav">
    	<ul>
...
<div class="wrapper">
    ...
    <!-- Content -->
    <div class="content">
    	<div class="title"><h5>Contribution Management</h5></div>
        
        <!-- Static table -->
        <div class="widget first">
        	<div class="head"><h5 class="iFrames">Contributor List</h5></div>
            <table cellpadding="0" cellspacing="0" width="100%" class="tableStatic">
            	<thead>
                	<tr>
                        <td width="20%">Project Title</td>
                        <td width="20%">Investment Req.</td>
                        <td width="20%">Investor</td>
                        <td width="20%">Invested Amount</td>
                        <td width="20%">Action</td>
                    </tr>
                </thead>
                <tbody>
                <?php
				$getQuery = mysql_query("SELECT * FROM investment ORDER BY id DESC");
				while($getValue = mysql_fetch_array($getQuery))
				{
				?>
                	<tr>
                        <td align="center">
						<?php $MyPitch = mysql_fetch_array(mysql_query("SELECT * FROM mypitch WHERE id = '".$getValue['pitchid']."'")); ?>
                        <strong><?php echo $MyPitch['title']; ?></strong> - 
						<?php echo $getValue['investid'];?>
                        </td>
                        <td align="center"><?php echo '$'.number_format($MyPitch['investment']); ?></td>
                        <td align="center">
                        <?php
						$GetUserDetails = mysql_fetch_array(mysql_query("SELECT * FROM register WHERE id = '".$getValue['userid']."'"));
						echo $GetUserDetails['username'];
						?>
                        </td>
                        <td align="center"><?php echo '$'.round($getValue['amount']); ?></td>
                        <td align="center"><!--<a href="#">Edit</a> / --><a href="contributors.php?DelId=<?php echo $getValue['id']; ?>" onclick="return confirm('Are you sure you want to delete?')">Delete</a></td>
                    </tr>
                    <?php
				}
					?>
