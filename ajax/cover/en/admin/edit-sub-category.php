...
{
	mysql_query("UPDATE `subcat` SET catid = '".$_REQUEST['category']."', subcat =  '".$_REQUEST['subcategory']."' WHERE id = '".$_REQUEST['eID']."'");
	header("location:edit-sub-category.php?eID=".$_REQUEST['eID']."");
	exit;
}
?>
...
<div class="wrapper">
    ...
    <div class="content">
        ...
        <form action="" class="mainForm" method="post" name="form1" id="form1">
          <?php $getCat = mysql_fetch_array(mysql_query("SELECT * FROM subcat where id = '".$_REQUEST['eID']."'")); ?>
        	<!-- Input text fields -->
            <fieldset>
            <div class="widget first">
            <div class="head"><h5 class="iList">Edit Sub-Category</h5></div>
            <div class="rowElem noborder">
            <label>Category</label>
            <div class="formRight">
                <select name="category" id="category">
					<?php $getCategory = mysql_query("SELECT * FROM `category` WHERE status = 'Y' ORDER by id ASC");
                    while($getNameCat = mysql_fetch_array($getCategory)) { ?>
                    <option <?php if($getCat['catid'] == $getNameCat['id']) { ?> selected="selected" <?php } ?> value="<?php echo $getNameCat['id']; ?>"><?php echo $getNameCat['catname']; ?></option>
                    <?php } ?>
                </select>
            ...
            <label>Category</label>
            <div class="formRight"><input type="text" name="subcategory" id="subcategory" value="<?php echo $getCat['subcat']; ?>" /></div><div class="fix"></div></div>
            <br />
