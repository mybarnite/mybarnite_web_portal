...
{
	mysql_query("DELETE FROM `wishlist` WHERE id = '".$_REQUEST['DelId']."' ");
...
<div class="wrapper">
    ...
    <div class="content">
      ...
      <div class="widget first">
       	<div class="head">
        	  <h5 class="iFrames">Manage Wishlists - <?php $getUser = mysql_fetch_array(mysql_query("SELECT * FROM `register` WHERE id = '".$_REQUEST['uid']."'")); ?><b><?php echo $getUser['firstname'].' '.$getUser['lastname']; ?></b></h5>
   	    </div>
          <table cellpadding="0" cellspacing="0" width="100%" class="tableStatic">
           	  <thead>
               	  <tr>
                      <td width="25%" style="padding-left:10px; text-align:left;"></td>
                      <td width="35%" style="padding-left:10px; text-align:left;">Product Name</td>
                      <td width="28%" align="center" valign="middle">Buyer Name</td>
              ...
              <tbody>
                	<?php 
					if($_REQUEST['uid']!='') { 
						$getQuery = mysql_query("SELECT * FROM `wishlist` WHERE uid = '".$_REQUEST['uid']."' ORDER BY id DESC");
					} else { 
						$getQuery = mysql_query("SELECT * FROM `wishlist` ORDER BY id DESC");
					}
					  while($getValue = mysql_fetch_array($getQuery)) { ?>
               	  <tr>
                      <td><?php $getProduct = mysql_fetch_array(mysql_query("SELECT * FROM `products` WHERE proid = '".$getValue['pid']."'")); ?><img src="../upload/product/<?php echo $getProduct['item_image1'];?>" width="160" alt="" /></td>
                      <td><?php echo $getProduct['product_name']; ?></td>
                      <td align="center" valign="middle"><?php $getBuyer = mysql_fetch_array(mysql_query("SELECT * FROM `register` WHERE id = '".$getValue['uid']."'")); echo $getBuyer['firstname'].' '.$getBuyer['lastname']; ?></td>
                      <td width="12%" align="center"><a href="wishlist.php?DelId=<?php echo $getValue['id']; ?>" onclick="return confirm('Are you sure you want to delete?')">Remove</a></td>
                  </tr>
                  <?php }	?>
              </tbody>
          </table>
