...
{
	mysql_query("INSERT INTO `saticpages` (`id`, `title`, `metatag`, `metakeyword`, `pagename`, `content`, `pagetype`, `status`) VALUES ('', '".$_REQUEST['title']."', '".$_REQUEST['metatag']."', '".$_REQUEST['metakeyword']."', '".$_REQUEST['pagename']."', '".mysql_real_escape_string($_REQUEST['content'])."', '".$_REQUEST['select']."', 'Y')"); header("location:manage-page.php?ret=1"); exit();
}
?>
            ...
            <fieldset>
                <div class="widget first">
                    <div class="head"><h5 class="iList">Create Page</h5></div>
                        <?php if(isset($_REQUEST['ret']) && $_REQUEST['ret'] == '1') { ?><div style="background-color:#e0edc1; color:#405802; padding:7px 8px 7px 8px; font-weight:bold; border:#FFF 2px solid;">Page Created Successfully!</div><?php } ?>
                        <div class="rowElem noborder">
                          <label>Page Title:</label>
                          <div class="formRight"><input name="title" type="text" id="title" placeholder="Enter Meta Title" /></div><div class="fix"></div></div>
                        <div class="rowElem">
                          <label>Meta Keyword:</label>
                          <div class="formRight"><input name="metatag" type="text" id="metatag" placeholder="Enter Meta Keyword" /></div><div class="fix"></div></div>
                        <div class="rowElem"><label>Meta Description:</label><div class="formRight"><input name="metakeyword" type="text" id="metakeyword" placeholder="Enter Meta Description" /></div><div class="fix"></div></div>
                    <div class="rowElem"><label>Page Name:</label><div class="formRight">
                    <input name="pagename" type="text" class="autoF" id="pagename" placeholder="Enter Page Name" /></div><div class="fix"></div></div>
                    
                    <!--<div class="rowElem noborder">
                        <label>Page Link Location :</label>
                        <div class="formRight">
                        <select name="select" >
                            <option value="">Choose Category</option>
                            <option value="HM">Header Menu (Max 3 Link)</option>
                            <option value="F">Footer Menu (No Limit)</option>
                            <option value="HE">Help Page (No Limit)</option>
                        </select>&nbsp;<span style="color:#F00;">*No selection will show link by default in footer</span>
                        </div>
                        <div class="fix"></div>
                    </div>-->
