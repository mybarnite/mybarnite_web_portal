...
{
	mysql_query("DELETE FROM `wefind_notify` WHERE id = '".$_REQUEST['DelId']."' ");
	header("location:wefind-notify.php");
	exit;
}
?>
...
<div class="wrapper">
    ...
    <div class="content">
        ...
        <div class="head">
        	  <h5 class="iFrames">WeFind Notifications</h5>
       	  	</div>
            <table cellpadding="0" cellspacing="0" width="100%" class="tableStatic">
            	<thead>
                	<tr>
                        ...
                        <td width="21%" align="center" valign="middle">Requested On</td>
                        <td width="15%" align="center" valign="middle">Action</td>
                    </tr>
                </thead>
                <tbody>
                <?php $getQuery = mysql_query("SELECT * FROM `wefind_notify` ORDER BY id DESC");
					  while($getProduct = mysql_fetch_array($getQuery)) { 
					  $getValue = mysql_fetch_array(mysql_query("SELECT * FROM `products` WHERE proid = '".$getProduct['proid']."'")); ?>
                	<tr>
                        <td colspan="2"><a href="edit-product.php?eID=<?php echo $getValue['proid']; ?>"><?php echo $getValue['product_name']; ?></a><br /><strong>Seller:</strong> <?php $getcategory = mysql_fetch_array(mysql_query("SELECT * FROM register WHERE id = '".$getValue['uid']."'")); ?><a href="seller-details.php?id=<?php echo $getcategory['id']; ?>"><?php echo $getcategory['company_name']; ?></a><br /><?php echo $getcategory['email']; ?><br /><?php echo $getcategory['phone']; ?></td>
                        <td width="25%" align="center" valign="middle"><img src="../upload/product/<?php echo $getValue['item_image1'];?>" width="160" alt="" /></td>
                      	<td width="21%" align="center" valign="middle"><?php $registerUser = mysql_fetch_array(mysql_query("SELECT * FROM register WHERE id = '".$getProduct['uid']."'")); ?><strong>Requested By</strong><br /><?php echo $registerUser['firstname'].' '.$registerUser['lastname']; ?><br /><strong>Email</strong><br /><?php echo $registerUser['email']; ?><br /><strong>Phone</strong><br /><?php echo $registerUser['phone']; ?><br /><strong>Requested On:</strong><br /><?php echo $getcategory['date']; ?></td>
                        <td width="15%" align="center" valign="middle"><a href="#" onclick="return confirm('Notify Mailing Service Currently disabled due to testing purpose...')">Notify Sellers</a><br /><a href="wefind-notify.php?DelId=<?php echo $getValue['proid']; ?>" onclick="return confirm('Are you sure you want to delete this notification?')">Delete</a></td>
                    </tr>
                    <?php } ?>
                </tbody>
            </table>
