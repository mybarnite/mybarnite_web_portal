...
{
	$Insert = mysql_query("INSERT INTO `midwest`.`outbox` (`id`, `aid`, `email`, `subject`, `message`, `date`, `time`, `datetime`, `status`) VALUES ('', '".$_SESSION['AdminID']."', '".$_REQUEST['email']."', '".$_REQUEST['subject']."', '".$_REQUEST['message']."', '".date("d M, Y")."', '".date("G:i A")."', now(), 'N')");
...
$headers .= 'From: Response from Midwest <support@midwest.com>' . "\r\n";

// Mail it
mail($to, $subject, $message, $headers);
	
header("location:sendEmail.php?ret=1");
exit;
}
?>
...
<div class="wrapper">
    ...
    <div class="content">
        ...
        <form action="" class="mainForm" method="post" name="form1" id="form1">
            ...
            <div class="head"><h5 class="iList">Send Email</h5></div>
            <?php if(isset($_REQUEST['ret']) == '1') { ?>
            <div style="color:#090; font-weight:bold; padding:5px 0 5px 13px;">Mail Sent Successfully!</div>
            <?php } ?>
            <div class="rowElem noborder">
            <label>To</label>
            <div class="formRight"><input name="email" type="text" id="email" value="<?php if(isset($_REQUEST['email'])) { echo $_REQUEST['email']; } ?>" />
            </div></div>
            <div class="rowElem noborder">
            <label>Subject</label>
            <div class="formRight"><input name="subject" type="text" id="subject" />
            </div></div>
            <div class="rowElem noborder">
            <label>Message</label>
            <div class="formRight">
            ...
            <br />
            <input type="submit" name="submit" value="SEND" class="greyishBtn submitForm" />
            <div class="fix"></div>
            </div>
            </fieldset>
