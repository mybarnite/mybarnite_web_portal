...
<div class="wrapper">
    ...
    <div class="content">
        ...
        <div style="padding:18px 0 0 0;">This report displays the total sales for a given day.</div>
        <!-- Static table -->
        <div class="widget first">
       	  <div class="head"><h5 class="iFrames">Daily Sales</h5></div>
            <table cellpadding="0" cellspacing="0" width="100%" class="tableStatic">
            	<thead>
                	<tr>
                      <td width="60%" align="left" valign="middle" style="text-align:left; padding-left:8px;"></td>
                        <td width="20%"># of Orders</td>
                        <td width="20%">Total Sales</td>
                    </tr>
                </thead>
                <tbody>
					<?php
                    $getPage = mysql_query("SELECT DISTINCT date FROM transaction ORDER by datetime DESC");
                        while($getArray = mysql_fetch_array($getPage))
                        {
                    ?>
                        <tr>
                            <td width="60%" align="left" valign="middle"><strong><?php echo $getArray['date']; ?></strong></td>
                            <td width="20%" align="center"><?php echo mysql_num_rows(mysql_query("SELECT * FROM transaction WHERE  date = '".$getArray['date']."'")); ?></td>
                            <td width="20%" align="center"><?php $countAmount = mysql_query("SELECT SUM(amount) FROM transaction WHERE date = '".$getArray['date']."'"); $cartArrayVal = mysql_fetch_array($countAmount); echo 'Rs.'.number_format($cartArrayVal[0]); ?></td>
                        </tr>
                    <?php
                        }
                    ?>
