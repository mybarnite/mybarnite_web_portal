...
{
	mysql_query("UPDATE `admin_group` SET `group_name` = '".$_REQUEST['group_name']."', `config` = '".$_REQUEST['config']."', `customer` = '".$_REQUEST['customer']."', `catalog` = '".$_REQUEST['catalog']."', `quotes` = '".$_REQUEST['quotes']."', `export` = '".$_REQUEST['export']."', `page` = '".$_REQUEST['page']."', `reports` = '".$_REQUEST['reports']."', `tool` = '".$_REQUEST['tool']."', status = '".$_REQUEST['status']."' WHERE id = '".$_REQUEST['eID']."'");	
	header("location:edit-group.php?eID=".$_REQUEST['eID']."&ret=1");
	exit;
}
?>
...
<div class="wrapper">
    ...
    <div class="content">
        ...
        <?php $getGroup = mysql_fetch_array(mysql_query("SELECT * FROM admin_group WHERE id = '".$_REQUEST['eID']."'")); ?>
        	<!-- Input text fields -->
            <fieldset>
            <div class="widget first">
            <div class="head"><h5 class="iList">Edit Group</h5></div>
            <?php if(isset($_REQUEST['ret']) == '1') { ?>
            <div style="color:#090; font-weight:bold; padding:5px 0 5px 13px;">Group Added Successfully!</div>
            <?php } ?>
            <div class="rowElem noborder">
            <label>Group Name</label>
            <div class="formRight"><input name="group_name" type="text" id="group_name" value="<?php echo $getGroup['group_name']; ?>" /></div><div class="fix"></div></div>
            <div style="padding:0 0 0 12px; font-weight:bold;">Edit Group Permission</div>
            <div class="rowElem noborder">
            <label>Configuration</label>
            <div class="formRight">
            <select name="config" id="config" style="padding:5px; width:100px;">
              <option <?php if($getGroup['config'] == 'N') { ?> selected="selected" <?php } ?> value="N">No</option>
              <option <?php if($getGroup['config'] == 'Y') { ?> selected="selected" <?php } ?> value="Y">Yes</option>
            </select></div><div class="fix"></div></div>
            <div class="rowElem noborder">
            <label>Customers</label>
            <div class="formRight">
            <select name="customer" id="customer" style="padding:5px; width:100px;">
              <option <?php if($getGroup['customer'] == 'N') { ?> selected="selected" <?php } ?> value="N">No</option>
              <option <?php if($getGroup['customer'] == 'Y') { ?> selected="selected" <?php } ?> value="Y">Yes</option>
            </select></div><div class="fix"></div></div>
            <div class="rowElem noborder">
            <label>Catalog</label>
            <div class="formRight">
            <select name="catalog" id="catalog" style="padding:5px; width:100px;">
              <option <?php if($getGroup['catalog'] == 'N') { ?> selected="selected" <?php } ?> value="N">No</option>
              <option <?php if($getGroup['catalog'] == 'Y') { ?> selected="selected" <?php } ?> value="Y">Yes</option>
            </select></div><div class="fix"></div></div>
            <div class="rowElem noborder">
            <label>Orders / Quotes</label>
            <div class="formRight">
            <select name="quotes" id="quotes" style="padding:5px; width:100px;">
              <option <?php if($getGroup['quotes'] == 'N') { ?> selected="selected" <?php } ?> value="N">No</option>
              <option <?php if($getGroup['quotes'] == 'Y') { ?> selected="selected" <?php } ?> value="Y">Yes</option>
            </select></div><div class="fix"></div></div>
            <div class="rowElem noborder">
            <label>Export Data</label>
            <div class="formRight">
            <select name="export" id="export" style="padding:5px; width:100px;">
              <option <?php if($getGroup['export'] == 'N') { ?> selected="selected" <?php } ?> value="N">No</option>
              <option <?php if($getGroup['export'] == 'Y') { ?> selected="selected" <?php } ?> value="Y">Yes</option>
            </select></div><div class="fix"></div></div>
            <div class="rowElem noborder">
            <label>EZ Pages</label>
            <div class="formRight">
            <select name="page" id="page" style="padding:5px; width:100px;">
              <option <?php if($getGroup['page'] == 'N') { ?> selected="selected" <?php } ?> value="N">No</option>
              <option <?php if($getGroup['page'] == 'Y') { ?> selected="selected" <?php } ?> value="Y">Yes</option>
            </select></div><div class="fix"></div></div>
            <div class="rowElem noborder">
            <label>Tools / Extras</label>
            <div class="formRight">
            <select name="tool" id="tool" style="padding:5px; width:100px;">
              <option <?php if($getGroup['tool'] == 'N') { ?> selected="selected" <?php } ?> value="N">No</option>
              <option <?php if($getGroup['tool'] == 'Y') { ?> selected="selected" <?php } ?> value="Y">Yes</option>
            </select></div><div class="fix"></div></div>
            <div class="rowElem noborder">
            <label>Reports</label>
            <div class="formRight">
            <select name="reports" id="reports" style="padding:5px; width:100px;">
              <option <?php if($getGroup['reports'] == 'N') { ?> selected="selected" <?php } ?> value="N">No</option>
              <option <?php if($getGroup['reports'] == 'Y') { ?> selected="selected" <?php } ?> value="Y">Yes</option>
            </select></div><div class="fix"></div></div>
            <div class="rowElem noborder">
            <label>Status</label>
            <div class="formRight">
            <select name="status" id="status" style="padding:5px; width:100px;">
              <option <?php if($getGroup['status'] == 'N') { ?> selected="selected" <?php } ?> value="N">No</option>
              <option <?php if($getGroup['status'] == 'Y') { ?> selected="selected" <?php } ?> value="Y">Yes</option>
            </select></div><div class="fix"></div></div>
