...
if(isset($_REQUEST['DelId'])!='')
{
	...
	header("location:low-stock-notify.php");
	exit;
}
?>
...
<div class="wrapper">
    ...
    <div class="content">
        ...
        <div class="head">
        	  <h5 class="iFrames">Low Stock Notification</h5>
       	  	</div>
            <table cellpadding="0" cellspacing="0" width="100%" class="tableStatic">
            	<thead>
                	<tr>
                        <td colspan="2" style="text-align:left; padding-left:8px;">Product Name</td>
                        <td width="27%" align="center" valign="middle">Seller Details</td>
                        <td width="16%" align="center" valign="middle">Action</td>
                    </tr>
                </thead>
                <tbody>
                <?php $getQuery = mysql_query("SELECT * FROM `products` WHERE qtynos < 3 ORDER BY proid DESC");
					  while($getValue = mysql_fetch_array($getQuery)) { ?>
                	<tr>
                        <td colspan="2"><a href="edit-product.php?eID=<?php echo $getValue['proid']; ?>"><?php echo $getValue['product_name']; ?></a><br /><strong>Seller Details:</strong> <?php $getcategory = mysql_fetch_array(mysql_query("SELECT * FROM register WHERE id = '".$getValue['uid']."'")); ?> <a href="seller-details.php?id=<?php echo $getcategory['id']; ?>"><?php echo $getcategory['company_name']; ?></a><br /><strong>Price / Quantity:</strong> <?php echo 'Rs.'.number_format($getValue['sale_rate'],2); ?> - <?php echo $getValue['qtynos']; ?></td>
                        <td width="27%" align="center" valign="middle"><img src="../upload/product/<?php echo $getValue['item_image1'];?>" width="160" alt="" /></td>
                      	<td width="16%" align="center" valign="middle"><a href="#" onclick="return confirm('Notify Mailing Service Currently disabled due to testing purpose...')">Notify</a> / <a href="manage-product.php?DelId=<?php echo $getValue['proid']; ?>" onclick="return confirm('Are you sure you want to delete this product?')">Delete</a></td>
                    </tr>
                    <?php } ?>
                </tbody>
            </table>
