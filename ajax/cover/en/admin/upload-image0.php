...
/*if(isset($_REQUEST['confirm']))
{
	...
	mysql_query("UPDATE `products` SET item_image1 = '".$FileName1."' WHERE proid = '".$_REQUEST['id']."'");
	...
	header("location:upload-image.php?ret=1&proid=".$_REQUEST['id']."");
	exit;
}
*/
if(isset($_REQUEST['proceed']) && $_REQUEST['color_name']!='')
{
	...
	header("location:another-product.php?id=".$_REQUEST['proid']."");
	exit;
}
...
<!-------- js & css ------>
<link rel="stylesheet" href="css/img_upload/dropzone.css" />

  <script src="js/img_upload/dropzone.js"></script>
<!------- End ------->
</head>
...
<div class="wrapper">
    ...
    <div class="content">
        ...
        <?php if($_REQUEST['ret'] == '') { ?>
        <!-- Static table -->
        <form action="upload.php" method="post"  name="form1" class="mainForm dropzone" id="form1 img_upload">
...
-->            <div id="drag">
                <center> <h1>Drag and Drop files </h1>
                <h5>When loading is complete image will be uploaded.</h5></center>
               
             <br>
                </div>
                
           </div>
           <input type="hidden" name="req_id" id="req_id" value="<?php echo $_REQUEST['id'];?>" />
        ...
        <?php $getProduct = mysql_fetch_array(mysql_query("SELECT * FROM `products` WHERE proid = '".$_REQUEST['proid']."'")); ?>
           ...
           <div class="rowElem noborder"><strong>Didn't find your preferred color in above dropdown?</strong></div>
            ...
            <br />
            <input type="submit" name="proceed" value=" Finish Upload " class="button" onClick="window.location='manage-product.php'" />&nbsp;&nbsp;<input type="submit" name="proceed" value="Continue &gt; &gt;" class="greyishBtn submitForm" />
            <div class="fix"></div>
            </div>
            </fieldset>
