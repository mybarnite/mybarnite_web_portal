...
$Sqldata = mysql_fetch_array(mysql_query("SELECT * FROM saticpages WHERE id = '".$_REQUEST['EditId']."'"));

if(isset($_REQUEST['submit']))
{
	$query = mysql_query("UPDATE `saticpages` SET `title` = '".$_REQUEST['title']."',
						`metatag` = '".$_REQUEST['metatag']."',
						`metakeyword` = '".$_REQUEST['metakeyword']."',
						`pagename` = '".$_REQUEST['pagename']."',
						`pagetype` = '".$_REQUEST['pagetype']."',
						`content` = '".mysql_real_escape_string($_REQUEST['content'])."' WHERE id = '".$_REQUEST['EditId']."'");
	header("location:edit-page.php?EditId=".$_REQUEST['EditId']."&ret=1");
	exit();
}
?>
...
<div class="wrapper">
    ...
    <!-- Content -->
    <div class="content">
    	<div class="title"><h5>General Settings</h5></div>
        ...
        <form action="" class="mainForm" method="post" name="form1" id="form1">
            ...
            <fieldset>
                <div class="widget first">
                    <div class="head"><h5 class="iList">Create Page</h5></div>
                    <?php if(isset($_REQUEST['ret']) && $_REQUEST['ret'] == '1') { ?>
            	<div style="background-color:#e0edc1; color:#405802; padding:7px 8px 7px 8px; font-weight:bold; border:#FFF 2px solid;">Informations Successfully Updated! </div>
					<?php } ?>
                        <div class="rowElem noborder">
                          <label>Page Title:</label>
                          <div class="formRight"><input name="title" type="text" id="title" placeholder="Enter Meta Title" value="<?php echo $Sqldata['title']; ?>" /></div><div class="fix"></div></div>
                        <div class="rowElem">
                          <label>Meta Keyword:</label>
                          <div class="formRight"><input name="metatag" type="text" id="metatag" placeholder="Enter Meta Keyword" value="<?php echo $Sqldata['metatag']; ?>" /></div><div class="fix"></div></div>
                        <div class="rowElem"><label>Meta Description:</label><div class="formRight"><input name="metakeyword" type="text" id="metakeyword" placeholder="Enter Meta Description" value="<?php echo $Sqldata['metakeyword']; ?>" /></div><div class="fix"></div></div>
                    <div class="rowElem"><label>Page Name:</label><div class="formRight">
                    <input name="pagename" type="text" class="autoF" id="pagename" placeholder="Enter Page Name" value="<?php echo $Sqldata['pagename']; ?>" /></div><div class="fix"></div></div>
                    ...
                    <!--<div class="rowElem noborder">
                        ...
                        <select name="pagetype">
                            ...
                            <option value="S"<?php if($Sqldata['pagetype'] == 'S') {?> selected="selected"<?php } ?>>Static Page</option>
                            <option value="H"<?php if($Sqldata['pagetype'] == 'H') {?> selected="selected"<?php } ?>>Help Page</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>-->
               ...
               <fieldset>
                <div class="widget">    
                    <div class="head"><h5 class="iPencil">Page Content</h5></div>
                    <textarea name="content" cols="" rows="18" id="content"><?php echo $Sqldata['content']; ?></textarea>                
                </div>
            </fieldset>
            <br />
...
<div id="footer">
	<div class="wrapper">
    	<span>&copy; Copyright <?php date("Y"); ?>. All rights reserved. Theme Designed by <a href="http://www.congruentsolution.com" title="" target="_blank">Congruent Solution Inc.</a></span>
    </div>
</div>
