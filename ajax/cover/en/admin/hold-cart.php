...
{
	mysql_query("DELETE FROM `mycart` WHERE id = '".$_REQUEST['DelId']."' ");
	header("location:hold-cart.php"); exit;
}
?>
...
<div class="wrapper">
    ...
    <div class="content">
        ...
        <div class="widget first">
        	<div class="head">
        	  <h5 class="iFrames">Hold in Cart</h5>
       	  </div>
            <table cellpadding="0" cellspacing="0" width="100%" class="tableStatic">
              ...
              <tbody>
                <?php $getQuery = mysql_query("SELECT * FROM `mycart` ORDER BY datetime DESC");
					  while($getValue = mysql_fetch_array($getQuery)) { ?>
               	  <tr>
                      <td><?php $getProduct = mysql_fetch_array(mysql_query("SELECT * FROM `products` WHERE proid = '".$getValue['proid']."'")); ?><img src="../upload/product/<?php echo $getProduct['item_image1'];?>" width="160" alt="" /></td>
                      <td><?php echo $getProduct['product_name']; ?></td>
                      <td align="center" valign="middle"><?php $getBuyer = mysql_fetch_array(mysql_query("SELECT * FROM `register` WHERE id = '".$getValue['uid']."'")); ?><strong><?php echo $getBuyer['firstname'].' '.$getBuyer['lastname']; ?></strong><br /><?php echo $getBuyer['email']; ?><br /><?php echo $getBuyer['phone']; ?></td>
                      <td width="12%" align="center"><a href="hold-cart.php?DelId=<?php echo $getValue['id']; ?>" onclick="return confirm('Are you sure you want to delete?')">Remove</a></td>
                  </tr>
                  <?php }	?>
              </tbody>
          </table>
