...
if(isset($_REQUEST['submit']) && $_REQUEST['product'] != '')
{
$getProInfo = mysql_fetch_array(mysql_query("SELECT * FROM products WHERE proid = '".$_REQUEST['product']."'"));
$Insert = mysql_query("INSERT INTO `product_tier` (`id`, `pid`, `sku`, `ask`, `name`, `quantity`, `datetime`, `status`) VALUES ('', '".$_REQUEST['product']."', '".$getProInfo['SKU']."', '".$_REQUEST['askprice']."', '".$_REQUEST['name']."', '".$_REQUEST['quantity']."', now(), 'Y')");
header("location:add-ties.php?ret=1");
exit;
}
?>
...
<div class="wrapper">
    ...
    <!-- Content -->
    <div class="content">
    	<div class="title"><h5>Catalog</h5></div>
        ...
        <form action="" class="mainForm" method="post" name="form1" id="form1">
            ...
            <div class="head"><h5 class="iList">Add Tier Price</h5></div>
            <?php if(isset($_REQUEST['ret']) == '1') { ?>
            <div style="color:#090; font-weight:bold; padding:5px 0 5px 13px;">Tier Added Successfully!</div>
            <?php } ?>
            <div class="rowElem noborder">
            <label>Product</label>
            <div class="formRight">
              <select name="product" id="product" style="padding:5px; width:350px;">
                <option value="">Select Product</option>
                <?php
				$getProduct = mysql_query("SELECT * FROM products ORDER by proid DESC");
				while($getProductArr = mysql_fetch_array($getProduct))
				{
				?>
                <option value="<?php echo $getProductArr['proid']; ?>"><?php echo $getProductArr['product_name']; ?></option>
                <?php } ?>
              </select>
            </div><div class="fix"></div></div>
            <div class="rowElem noborder">
            	<label>Ask Price</label>
            	<div class="formRight"><input name="askprice" type="text" id="askprice" /></div><div class="fix"></div></div>
            <div class="rowElem noborder">
            	<label>Name</label>
            	<div class="formRight"><input name="name" type="text" id="name" /></div><div class="fix"></div></div>
            <div class="rowElem noborder">
            	<label>Quantity</label>
            	<div class="formRight"><input name="quantity" type="text" id="quantity" /></div><div class="fix"></div></div>
            <br />
            <input type="submit" name="submit" value="Add Tier" class="greyishBtn submitForm" />
            <div class="fix"></div>
            </div>
            </fieldset>
