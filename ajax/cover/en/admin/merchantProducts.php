...
{
	mysql_query("DELETE FROM products WHERE proid = '".$_REQUEST['DelId']."' ");
...
<div class="wrapper">
    ...
    <div class="content">
        ...
        <div class="widget first">
        	<div class="head"><h5 class="iFrames">Seller Products</h5></div>
            <table cellpadding="0" cellspacing="0" width="100%" class="tableStatic">
            	<thead>
                	<tr>
                        <td width="44%" style="text-align:left; padding-left:8px;">Product Name</td>
                        <td width="25%" style="text-align:left; padding-left:8px;">&nbsp;</td>
                      <td width="16%" align="center" valign="middle">Price</td>
                        <td width="15%" align="center" valign="middle">Action</td>
                    </tr>
                </thead>
                <tbody>
                <?php $getSeller = mysql_query("SELECT * FROM register WHERE type != 'U' ORDER BY id DESC");
				while($getSellerArr = mysql_fetch_array($getSeller)) { ?>
                
                <?php $getQuery = mysql_query("SELECT * FROM products WHERE uid = '".$getSellerArr['id']."'");
					  while($getValue = mysql_fetch_array($getQuery)) { ?>
                	<tr>
                        <td><?php echo $getValue['product_name']; ?><br /><strong>Sub-Category:</strong> <?php $getsubcatName = mysql_fetch_array(mysql_query("SELECT * FROM subcat WHERE id = '".$getValue['subcategory']."'")); echo $getsubcatName['subcat']; ?></td>
                        <td><img src="../upload/product/<?php echo $getValue['item_image1'];?>" width="160" alt="" /></td>
                        <td width="16%" align="center" valign="middle"><?php echo 'Rs.'.number_format($getValue['sale_rate']); ?></td>
                        <td width="15%" align="center" valign="middle"><a href="edit-product.php?eID=<?php echo $getValue['proid']; ?>">Edit</a> / <a href="merchantProducts.php?DelId=<?php echo $getValue['proid']; ?>" onclick="return confirm('Are you sure you want to delete?')">Delete</a></td>
                    </tr>
