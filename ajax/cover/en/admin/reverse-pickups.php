...
{
	mysql_query("DELETE FROM `reverse_pickup` WHERE reverseid = '".$_REQUEST['DelId']."' ");
	header("location:reverse-pickups.php"); exit;
}
?>
...
<div class="wrapper">
    ...
    <div class="content">
        ...
        <div class="widget first">
       	  <div class="head"><h5 class="iFrames">Reverse Pickups</h5></div>
            <table cellpadding="0" cellspacing="0" width="100%" class="tableStatic">
            	<thead>
                	<tr>
                      	<td width="36%" align="left" valign="middle" style="text-align:left; padding-left:8px;">Product Details</td>
                        <td width="26%" align="center" valign="middle">Buyer Details</td>
                      <td width="25%" align="center" valign="middle">Seller Details</td>
                        <td width="13%" align="center" valign="middle">Action</td>
                    </tr>
                </thead>
                <tbody>
                	<?php $getPage = mysql_query("SELECT * FROM reverse_pickup ORDER by datetime DESC"); ?>
                    <?php if(mysql_num_rows($getPage) > 0){ while($getArray = mysql_fetch_array($getPage)) { ?>
						<?php $productid = mysql_fetch_array(mysql_query("SELECT * FROM products WHERE proid = '".$getArray['productid']."'")); ?>
                        <?php $sellerid = mysql_fetch_array(mysql_query("SELECT * FROM register WHERE id = '".$getArray['sellerid']."'")); ?>
                        <?php $buyerid = mysql_fetch_array(mysql_query("SELECT * FROM register WHERE id = '".$getArray['buyerid']."'")); ?>
                    <tr>
                   	  <td width="36%" align="left" valign="middle" style="text-align:left; padding-left:8px;"><a href="details.php?id=<?php echo $sellerid['productid']; ?>" target="_blank"><?php echo $productid['product_name']; ?></a></td>
                        <td width="26%" align="center" valign="middle"><?php echo $sellerid['company_name']; ?></td>
                        <td width="25%" align="center" valign="middle"><?php echo $buyerid['firstname'].' '.$buyerid['lastname']; ?></td>
                        <td width="13%" align="center" valign="middle"><a href="#" onclick="return confirm('Oops Sorry! This action you can not proceed in demo entry.')">Update</a><br /><a href="reverse-pickups.php?DelId=<?php echo $sellerid['reverseid']; ?>" onclick="return confirm('Are you sure you want to delete?')">Delete</a></td>
                    </tr>
                	<?php } } else { ?>					
                    <tr>
                        <td height="100" colspan="3" align="center" valign="middle"><strong>No Reverse Pickups Request Found.</strong></td>
                    </tr>
                    <?php } ?>
                </tbody>
            </table>
